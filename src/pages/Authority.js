  ...
  useEffect(() => {
    ...
    if (seenFilter !== 'all') {
      filtered = filtered.filter(report => {
        // For 'seen' filter: show only reports that are seen but NOT resolved
        if (seenFilter === 'seen') {
          return report.status === 'Seen' && report.progress !== 'Resolved';
        } 
        // For 'unseen' filter: show only reports that are unseen
        else {
          return report.status !== 'Seen';
        }
      });
    }
